                                            ...
                                            <form class="simple-example"  method="post" id="upload_form" enctype="multipart/form-data" novalidate>
                                                ...
                                                <div class="form-row">
                                                    <div class="col-md-12 mb-4">
                                                        ...
                                                        <input type="text" readonly class="form-control" value="{{$podcast->name}}" placeholder="" value="" required>
                                                    ...
                                                    <div class="col-md-12 mb-4">
                                                        <label for="id">Audio Name</label>
                                                        
                                                        <select class="form-control  basic"  id="id" name="id">
                                                            <option >Audio</option>
                                                            @foreach(App\Models\Audio::where('user_id',Auth::User()->id)->whereNull('podcast_id')->get() as $audio)
                                                            <option v>Audio</option>
                                                                <option value="{{$audio->id}}">{{$audio->title}}</option>
                                                                @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <button class="btn btn-primary submit-fn mt-2" id="addBtn" onclick="addData()" type="submit">Add Audio</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
    ...
    function clearData(){
     ...
     function allData() {
        ...
        $.ajax({
          type: "GET",
          dataType: "json",
          url: "/user/flexflix/podcast/all-audio-show/"+id,
          ...
          success:function (response) {
            ...
            $.each(response, function(key,value){
              ...
              data = data + "<tr>"
              data = data + "<td>" +value.audio.title+ "</td>"
      ...
      $.ajax({
       url:"{{ url('/user/flexflix/podcast/audio-store') }}",
       method:"POST",
