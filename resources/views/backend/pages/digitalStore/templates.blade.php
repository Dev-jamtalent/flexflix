                                            ...
                                            <form class="simple-example"  method="post" id="upload_form" enctype="multipart/form-data" novalidate>
                                                ...
                                                <div class="form-row">
                                                    <div class="col-md-12 mb-4">
                                                        ...
                                                        <input type="text" readonly class="form-control" value="{{$digitalstore->name}}" placeholder="" value="" required>
                                                        
                                                        <div class="invalid-feedback">
                                                            Please fill the Category name
                                                        </div>
                                                    </div>
                                                    <input type="hidden" name="digital_store_id"id="digital_store_id"value="{{$digitalstore->id}}">
                                                    <div class="col-md-12 mb-4">
                                                        <label for="id">Template Name</label>
                                                        
                                                        <select class="form-control  basic"  id="id" name="id">
                                                            @foreach(App\Models\Template::where('user_id',Auth::User()->id)->get() as $template)
                                                    <option value="{{$template->id}}">{{$template->title}}</option>
                                                    @endforeach
                                            </select>
                                                    </div>
                                                </div>
                                                <button class="btn btn-primary submit-fn mt-2" id="addBtn" onclick="addData()" type="submit">Add Template</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
	...
	function clearData(){
     ...
     function allData() {
        ...
        $.ajax({
          type: "GET",
          dataType: "json",
          url: "/user/flexflix/digital-store/all-teplate-show/"+id,
          ...
          success:function (response) {
            ...
            $.each(response, function(key,value){
              ...
              data = data + "<tr>"
              data = data + "<td>" +value.template.title+ "</td>"

              data = data + "<td>"
              data = data + "<a  href =  class = 'btn btn-primary mr-2 btn-sm'> Delete </a>"
              data = data + "</td>"
              data = data + "</tr>"
            })
            $('tbody').html(data);
      ...
      $.ajax({
       url:"{{ url('/user/flexflix/digital-store/template-store') }}",
       method:"POST",
